...
import { deltaTime, Fn, If, instanceIndex, int, normalLocal, positionLocal, storage, transformNormalToView, vec3, vec4, greaterThanEqual, Return } from "three/tsl";
...
export function ECS_NBRGrid(_props: TerrainScatterProps) {
    ...
    const computeUpdate = useMemo(() => {
        return Fn(() => {
            ...
            offset.assign(offset.addAssign(vec3(deltaTime.mul(4), 0, 0)));
        })().compute(count);
    ...
    // NBR GRID --------------------------------------
    const grid = useMemo(() => {
        return new NeighbourGrid2D(100, 5, 16)
    }, [])

    const writeToGridCompute = useMemo(() => {
        return Fn(() => {
            // Return If Greter
    ...
    }, [instanceMatrix, count, grid]);

    useFrame(async () => {
        renderer.compute(grid.clearCompute())
        renderer.compute(writeToGridCompute)
        renderer.compute(grid.computeMirror())
